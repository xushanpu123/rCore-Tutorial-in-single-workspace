...
​       与rcore_tutorial不同，rcore_tutorial_in_single_workspace把各个实验所使用的公共模块进行了抽离，成为了独立的模块供各个chapter使用，各个chapter只保留了该章节所需要的基本功能和逻辑，其它模块则有着高度的抽象化特征，在移植polyhal和支持多架构的过程中，本项目需要分别对各个chapter和crate的代码进行polyhal和多架构的适配工作。因为这个特征，本文档以实现的功能分类，分别介绍各个功能如何进行具体的适配操作。

### 相关命令和测试：

在<arch>架构下运行章节<ch>，<arch>为riscv64，x86_64,aarch64,loongarch64四种架构中的一种，<ch>为1-8之一的数字，代表运行不同章节的内核和测例。
...
由于polyhal还在不断更新中，为了防止出现冲突，本项目fork了一份polyhal，并且需要指定特定的feature，kcontext表示可以使用内核上下文接口，trap表示具有中断机制和可以使用中断模块，boot表示自启动。值得注意的是，各个模块均需要引入polyhal，且各个模块引入polyhal时必须使用同样的链接和feature，否则就会造成一份代码链接多份进而产生链接bug。
...
在支持地址空间之前，原实验并没有开启页映射机制，因为指令采用物理内存地址来访问内存，并且系统和应用都可以无权限限制的访问任意物理内存地址。然而，由于利用polyhal启动时，polyhal会自动开启页机制，因此没有映射的空间并不能够直接使用，没有给予用户访问权限的空间也不能由用户态程序直接访问，因此，在任务加载时，本项目必须先对对应的内存地址映射，并给予用户访问权限，这样才能正常加载和使用这些程序。

为了实现这个功能，本项目提前移植了页帧分配器和堆分配器，并修改了加载时的代码：
...
而在ch3，多道程序设计的实验中，本项目还需要考虑给不同的任务分配不同的页面和栈空间：
...
rcore_tutorial_in_single_workspace的主程序逻辑对于每个模块的引入都有显式的初始化过程，若不对对应模块进行初始化，则无法使用对应模块提供的功能。同时，它使用异界传送门作为内核程序和用户程序之间连接的跳板，异界传送门实际上是内核中分配出来的一块特定的区域，任何内核程序都通过跳转到异界传送门后返回用户程序。另外，rcore_tutorial_in_single_workspace是通过一个大的循环语句来统一多任务调度切换和中断异常处理的，其基本结构如下面代码所示：
...
为了适配polyhal提供的接口，本项目在不破坏其基本逻辑结构的前提下，对代码进行了改写，主要修改方式如下：

1、不再在polyhal::interrupt标记处进行中断处理，而是在返回内核后，利用run_user_task()的返回值进行中断类型的判断和中断（以及信号）处理；

2、为了适配调度接口context_switch_pt()，本项目把内核模块变成了多处上下文函数，然后利用任务切换接口来进行跳转；
...
对于中断处理过程，也需要大量的修改，首先，移植过程中，本项目删除了所有原代码中内核态跳转到用户态相关的代码，因为polyhal提供的run_user_task()接口已经提供了相应的功能，对于具体如何从内核态返回用户态，上述的代码框架已经有所体现。除此之外，在移植过程中，本项目必须把所有中断帧结构都替换成polyhal提供的TrapFrame结构，并对所有初始化的行为都修改为对TrapFrame的初始化。

简单介绍一下TrapFrame的定义和基本操作，TrapFrame在polyhal中的定义为：
...
其中x保存了中断时的寄存器值，sepc(吐槽一下这个命名，一般只有riscv才有这个寄存器)代表了该中断结束时PC应当恢复的值，无论这个值的保存和恢复在原架构中是依靠特定寄存器实现的，还是压栈退栈实现的，polyhal都提供了忽略硬件实现细节访问和修改这个值的接口。与此同时，polyhal提供了TrapFrameArgs数据类型，来更方便的对TrapFrame进行访问（未注释的部分与本项目无关）：
...
下面的代码就展示了thread_create()时对其初始的TrapFrame类型的构建：
...
在本项目中，需要把所有记录中断上下文的结构都修改成上述形式。

### 4.系统调用的收集和处理和地址空间模块的适配

本小节只考虑系统调用在内核态下的处理。在本项目的处理中，本项目利用run_user_task()提供的返回值esr来判断中断类型，它是一个自定义数据类型EscapeReason，记录了任务从用户态进入内核态的原因，如果该原因为EscapeReason::Syscall，则说明触发了系统调用，而系统调用的类型和参数会保存在中断帧中，因此访问中断帧TrapFrame中的参数即可获取系统调用的具体类型和参数：
...
例如实现将页表映射时，本项目只需要调用polyhal::PageTable结构自带的map_page接口即可。
...
首先，原实验的内核空间采用的是对等映射，即内核空间中逻辑地址和物理地址是等价的，但是polyhal构建的内核采用的是高半核机制，即内核空间位于整体地址空间的高半核，内核的逻辑地址与物理地址有一个VIRT_ADDR_START大小的偏移量，基于这一点本项目需要修改VirtioHal特性中的地址相关内容：
...
最后，x86_64和loogarch64并不能通过MMIO方式访问VirtIOBlock，因此本项目为它提供了一个逻辑化的块设备RamdiskBlock：
...
impl BlockDevice for RamDiskBlock {
    fn read_block(&self, sector_offset: usize, buf: &mut [u8]) {
        assert!(buf.len() == 0x200, "block size is not 0x200");
        let rlen = buf.len();
        if (sector_offset * 0x200 + rlen) >= self.size {这样，就由具体的handle函数来完成系统调用的处理了，由于具体的系统调用处理属于不同的子系统，所以这里不做赘述。
            panic!("can't out of ramdisk range")
        };, buf: &[u8]) {
        let wlen = buf.len();
...
1、在信号模块中，返回用户态时可能需要跳转到自定义函数位置去执行接口，因此UserSignal类型需要维护中断帧上下文来实现这一功能，因此需要用TrapFrame来替换：
...
2、在sync/up.rs模块，需要访问riscv架构下的sie寄存器，这里替换成polyhal：：IRQ下提供的接口来实现：
...
3、时钟中断需要获取时间，在接入polyhal后，利用polyhal::Time模块提供的接口来支持：
...
原生的rcore_tutorial_in_single_workspace依靠xtask来实现编译和运行内核，由于该工具比较难以维护和使用，因此本项目使用python重新写了一个编译和运行脚本来支持多架构编译和运行。
...
接下来需要为user任务发起系统调用提供不同架构的支持，因为它们使用的trap指令是架构相关的，该代码位于syscall模块的user.rs中，以6个参数的系统调用发起函数syscall6()为例：
...
CI代码在.github/wokrflows/test.yml中，实际上就是运行了四种架构下的usertests，不过这里比较特殊的是loongarch64架构，polyhal并不支持该架构的停机自动退出，所以CI无法获取最终结果，因此本项目不断的检测usertests的输出，出现usertests Passed！即返回成功即可，相关代码如下：
